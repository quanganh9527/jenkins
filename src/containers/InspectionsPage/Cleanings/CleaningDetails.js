...
const pathCleaningList = "/cleaning";
const img = {
  display: "block",
  width: "auto",
  height: "100%",
...
function CleaningDetail({ intl, inspectionId }) {
  ...
  let { user } = userState;
  user = user || {};
  const isEdit = allowEditInspection(user, "Cleaning", inspection.status);
  ...
  useEffect(() => {
    ...
    dispatch(
      inspectionActions.getInspection(
        inspectionId,
        inspectionServices.roleByJobType.cleaning,
        user
      ),
    );
  ...
  const onSendConfirmComplete = () => {
    ...
    dispatch(
      inspectionActions.submitcompletedCleaning(inspection._id, paramsData),
    );
        ...
        {inspection.status === INSPECTION_STATUS.INPROGRESS && (
          <div className="d-flex align-items-center text-capitalize">
            <Button
              ...
              color="outline-link"
              className="mr-2"
              onClick={() => history.push(pathCleaningList)}
            >
  ...
  //     : [];
  /**
   * Em Dinh: https://infodation.atlassian.net/browse/EEAC-595?oldIssueView=true
   */
    ...
    (inspectionStatus) => {
      ...
      dispatch(
        reduxActions.updateInspection(
          updatedData,
          inspection.id,
          inspectionServices.jobTypes.cleaning,
          inspectionStatus,
        ),
      );
    },
      ...
      <PageTitle
        heading={
          viewSum || inspectionStatus === INSPECTION_STATUS.READY
            ...
            : intl.formatMessage({ id: "components.formTitle.cleaningDetails" })
        }
        ...
        <Col md="12">
          <Card className="main-card mb-3">
            ...
            <CardBody>
              ...
              {notificationCleaning && notificationCleaning.message ? (
                <Alert
                  color={
...
export default memo(injectIntl(CleaningDetail));
