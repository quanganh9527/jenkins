...
function ConfirmComplete({ isOpen, onToggleOpen, onSendConfirmComplete }) {
  return (
    <Modal isOpen={isOpen} backdrop={true}>
      <ModalHeader><FormattedMessage id="components.dialog.completeInspection" /></ModalHeader>
      <ModalBody>
        <FormGroup>
          <Label><FormattedMessage id="components.dialog.completeInspection.desc" /> {moment().format('DD/MM/YYYY')}</Label>
        </FormGroup>
        <FormGroup>
