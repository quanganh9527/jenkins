...
import { validationInspectionReject } from "./validation";
...
function ConfirmReject({ isOpen, onToggleOpen, inspection, intl }) {
  ...
  const { handleSubmit, register, errors, clearError, setValue } = useForm({
    validationSchema: validationInspectionReject(intl),
  });
  ...
  const onSubmitRejectInspection = values => {
    ...
    dispatch(
      inspectionActions.submitRejectInspection(
        inspection._id,
  ...
  return (
    <Modal isOpen={isOpen} backdrop={true}>
      <ModalHeader>
        <FormattedMessage id="components.dialog.rejectInspection" />
      </ModalHeader>
      <ModalBody>
        <FormGroup>
          <TextareaAutosize
            ...
            placeholder={intl.formatMessage({
              id: "components.dialog.rejectInspection.desc",
            })}
            onChange={({ target }) => {
