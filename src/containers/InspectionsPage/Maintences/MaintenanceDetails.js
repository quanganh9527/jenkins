...
import { map, find, cloneDeep, forEach, includes, filter } from "lodash";
import { useHistory } from "react-router-dom";
import Select from "react-select";
// import DropdownTreeSelect from "react-dropdown-tree-select";
/**
 * Em Dinh: https://infodation.atlassian.net/browse/EEAC-594?oldIssueView=true
 */
...
import Uploader from "../InspectionEdit/Uploader";
import "../InspectionEdit/styles.scss";
...
import ViewSummary from "./ViewSummaryMaintenance";
import ConfirmCompleteModal from "./ModalConfirmCompleteMaintenance";
import ModalConfirmReject from "./ModalConfirmRejectMaintenance";
...
import { RECEIVE_MESSAGE_MAINTENANCE } from "../constants";
...
function MaintenanceDetail({ intl, inspectionId }) {
  ...
  let { user } = userState;
  user = user || {};
  const isEdit = allowEditInspection(user, "Maintenance", inspection.status);
  ...
  useEffect(() => {
    ...
    // dispatch(inspectionActions.getContacts());
    // dispatch(inspectionActions.getDebtorContacts());
    // reset inspection, location
    ...
    dispatch(
      inspectionActions.getInspection(
        inspectionId,
        inspectionServices.roleByJobType.maintenance,
        user
      ),
    );
  ...
  const onHandleViewSummary = () => {
    ...
    forEach(inspectionItems, (inspectionItem) => {
      ...
      if (
        ...
        if (!isInvalid) {
          if (
            inspectionItem.actions &&
            inspectionItem.actions.length > 0 &&
            find(inspectionItem.actions, (actionItem) => !actionItem)
          ) {
            isInvalid = true;
            unitIndex = inspectionItem.unitIndex;
            return;
  ...
  const onSendConfirmComplete = () => {
    ...
    dispatch(
      inspectionActions.submitcompletedMaintenance(inspection._id, paramsData),
    );
  ...
  const _renderButtonsOnViewSummary = () => (
    <>
      ...
      >
        {intl.formatMessage({ id: "components.button.back" })}
      </Button>
      <Button
        size="sm"
        color="success"
      ...
      >
        {intl.formatMessage({ id: "components.button.complete" })}
      </Button>
    </>
  );
    ...
    <>
      ...
      {isReviewInspection && (
        <>
          ...
          >
            {intl.formatMessage({ id: "components.button.reject" })}
          </Button>
          <Button
            size="sm"
            color="secondary"
          ...
          >
            {intl.formatMessage({ id: "components.button.viewSummary" })}
          </Button>
        </>
      )}
    </>
      ...
      <>
        {inspection.status === INSPECTION_STATUS.INPROGRESS && (
          <div className="d-flex align-items-center text-capitalize">
            ...
            >
              {intl.formatMessage({ id: "components.button.cancel" })}
            </Button>
            <Button
              size="sm"
              color="success"
        ...
        {(inspection.status === INSPECTION_STATUS.COMPLETED ||
          ...
          >
            {intl.formatMessage({ id: "components.button.report" })}
          </Button>
        )}
        {inspection.status === INSPECTION_STATUS.READY && (
          <>
            ...
            {isReviewInspection && (
              ...
              >
                {intl.formatMessage({ id: "components.button.review" })}
              </Button>
            )}
          </>
        )}
        {inspection.status === INSPECTION_STATUS.OPEN && (
          <div className="d-flex align-items-center text-capitalize">
            ...
            <Button
              ...
              disabled={!isDirty}
            >
              {intl.formatMessage({ id: "components.button.save" })}
            </Button>
          </div>
        )}
      </>
    ...
    (inspectionStatus) => {
      ...
      dispatch(
        reduxActions.updateInspection(
          updatedData,
          inspection.id,
          inspectionServices.jobTypes.maintenance,
          inspectionStatus,
        ),
      );
    },
      ...
      <PageTitle
        heading={
          viewSum || inspectionStatus === INSPECTION_STATUS.READY
            ...
            : intl.formatMessage({
                id: "components.formTitle.maintenanceDetails",
              })
        }
            ...
            <CardBody>
              {!isValidInspection || !isValid ? (
                <Alert color={"danger"} isOpen={!isValidInspection || !isValid}>
                  {intl.formatMessage({
                    id: "pages.maintenance.alertRequiredData.review",
                  })}
                </Alert>
              ) : null}

              {notificationMaintenance && notificationMaintenance.message ? (
                <Alert
                  color={
                      ...
                      <Row form>
                        <Col md={6}>
                          <FormGroup>
                            <Label for="">
                              {intl.formatMessage({
                                id: "pages.inspection.unitAndSubUnit",
                              })}
                            </Label>
                            <Select
                              placeholder="Units"
                      ...
                      <Row>
                        ...
                        <Col md={12}>
                          <FormGroup>
                            <Label for="">
                              {intl.formatMessage({
                                id: "pages.inspection.internalWork",
                              })}
                            </Label>
                            <TextareaAutosize
                              className="form-control"
                              ...
                              maxRows={6}
                              maxLength={250}
                              placeholder="Internal work instructions description"
                              value={instructions}
                              onChange={(event) =>
                                                    ...
                                                    {includes(
                                                      ...
                                                      (!inspectionItem.images ||
                                                        ...
                                                        >
                                                          {intl.formatMessage({
                                                            id:
                                                              "pages.maintenance.errors.findingsImages.require",
                                                          })}
                                                        </Alert>
                                                      )}
                                                    {limitFilesError[
                                                                    ...
                                                                    !billItem.description && (
                                                                      <FormFeedback className="d-block">
                                                                        {intl.formatMessage(
                                                                          {
                                                                            id:
                                                                              "pages.maintenance.errors.billDes.require",
                                                                          },
                                                                        )}
                                                                      </FormFeedback>
                                                                    )}
                                                                        ...
                                                                        !billItem.costtype && (
                                                                          <FormFeedback className="d-block">
                                                                            {intl.formatMessage(
                                                                              {
                                                                                id:
                                                                                  "pages.maintenance.errors.costType.require",
                                                                              },
                                                                            )}
                                                                          </FormFeedback>
                                                                        )}
                                                                        ...
                                                                        !billItem.price && (
                                                                          <FormFeedback className="d-block">
                                                                            {intl.formatMessage(
                                                                              {
                                                                                id:
                                                                                  "pages.maintenance.errors.price.require",
                                                                              },
                                                                            )}
                                                                          </FormFeedback>
                                                                        )}
                                                                        ...
                                                                        !billItem.quantity && (
                                                                          <FormFeedback className="d-block">
                                                                            {intl.formatMessage(
                                                                              {
                                                                                id:
                                                                                  "pages.maintenance.errors.quantity.require",
                                                                              },
                                                                            )}
                                                                          </FormFeedback>
                                                                        )}
...
export default memo(injectIntl(MaintenanceDetail));
