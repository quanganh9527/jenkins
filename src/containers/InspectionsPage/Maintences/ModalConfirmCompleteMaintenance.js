...
function ConfirmComplete({ isOpen, onToggleOpen, onSendConfirmComplete }) {
  return (
    <Modal isOpen={isOpen} backdrop={true}>
      <ModalHeader><FormattedMessage id="pages.maintenance.dialog.completeMaintenance" /></ModalHeader>
      <ModalBody>
        <FormGroup>
          <Label><FormattedMessage id="pages.maintenance.dialog.completeMaintenance.des" /> {moment().format('DD/MM/YYYY')}</Label>
        </FormGroup>
        <FormGroup>
